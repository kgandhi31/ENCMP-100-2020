%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Course: ENCMP 100
% Assignment: 4B
% Name: Krish Gandhi
% CCID: gandhi
% U of A ID: 1621641
%
% Acknowledgements:
% None
%
% Description:
% This program calculates medal standings for Olympics.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [] = Assign4B_1621641()

clear; clc; close all;

% loads countries, gold, silver, and bronze variables
% country codes include XXX, which is used to handle tied medals
load olympics.mat gold silver bronze countries;

% create a structure array that contains the following fields
% country, number of gold medals, number of silver medals,
% number of bronze medals, total
temp_struc = struct('Country', '', 'Gold', [], 'Silver', [], 'Bronze', ...
    [], 'Total', []);

% preallocating a 27 element array of temp structures
results = repmat(temp_struc, 1, 27);

% compute medals for each country save into a structure array
for j=1:1:length(countries)
    results(j) = compute_medals(gold,silver,bronze,countries(j,:));
end

% display medal counts for all countries
print_country_results(results);

% display countries with
% most medals (i=1), most gold medals (i=2), >=20 medals (i=3)
for i=1:3
    print_best_countries(results,i);
end
end

function results = compute_medals(gold,silver,bronze,country)
% computes number of gold, silver, bronze medals and total medal tally for
% a given country and a given sport type
gold_count = 0;
silver_count = 0;
bronze_count = 0;
for j=1:1:length(gold)
    if country == gold(j,:)
        gold_count = gold_count + 1;
    end
    if country == silver(j,:)
        silver_count = silver_count + 1;
    end
    if country == bronze(j,:)
        bronze_count = bronze_count + 1;
    end
end
total_medals = gold_count + silver_count + bronze_count;
results = struct('Country', country, 'Gold', gold_count, 'Silver', ...
    silver_count, 'Bronze', bronze_count, 'Total', total_medals);
end

function [] = print_country_results(results)
% prints formatted results
fprintf('Country Gold Silver Bronze Total\n');
for i=1:1:length(results)
    if (results(i).Country ~= 'XXX')
        fprintf('%7s %4d %6d %6d %5d\n', results(i).Country, ...
            results(i).Gold, results(i).Silver, results(i).Bronze, ...
            results(i).Total);
    end
end
end

function [] = print_best_countries(results,selection)
% prints best performing countries
% selection = 1 shows countries with most medals
% selection = 2 shows countries with most gold medals
% selection = 3 shows countries with at least 20 medals
switch selection
    case 1
        [value,index] = max([results.Total]);
        fprintf('Countries with the most medals: ');
        fprintf('%s ', results(index).Country);
    case 2
        [value,index] = max([results.Gold]);
        fprintf('Countries with the most gold medals: ');
        fprintf('%s ', results(index).Country);
    case 3
        index = [results.Total] >= 20;
        fprintf('Countries with at least 20 medals: ');
        fprintf('%s ', results(index).Country);
end
fprintf('\n');
end